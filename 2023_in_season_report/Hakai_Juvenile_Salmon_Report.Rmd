...
# Before running this code, search for the previous years text (ie. 2022) in figure captions and replace with the actual year ie. 2023. 
...
# Create list of authors ordered by n field days
# Use map to apply the function to each element of the vector
all_names <- surveys_current$crew %>% 
  map(~strsplit(.x, ",")[[1]]) %>% 
  map(trimws)

all_names_flat <- unlist(all_names)
crew <- table(all_names_flat)
crew <- as.data.frame(crew)
crew <- crew[order(-crew$Freq), ]
crew <- tibble(crew)
crew$all_names_flat <- recode(crew$all_names_flat, "Tim Van Der Stap" = "Tim van der Stap")
...
To provide a rapid end of season summary of juvenile salmon migration characteristics and oceanographic conditions in the Discovery Islands and northern Strait of Georgia in British Columbia, Canada. 
...
2) Discovery Islands – highly-mixed low-plankton-biomass zone, and area of, historically, high potential for wild and farmed salmon interactions.
...
Field Crew: `r crew$all_names_flat`

\newpage

# Catches and Migration Timing
...
```{r cumulative catch timing, fig.cap = "Migration timing of juvenile sockeye salmon in the Discovery Islands in 2023 compared to 2015-2022 modelled using a logistic growth curve fitted to cumulative catch for each period."}
dbl_effort_2019 <- survey_seines_GH %>% 
...
tidy_catch <- select(survey_seines_GH, survey_date, survey_id, region, so_total) |> 
...
dbl_effort_2019 <- dbl_effort_2019 %>% 
  ...
  ungroup() %>% 
  select(survey_date, so_total)

# Calculate cumulative sum and daily percent within each year. Fit line through that
c_catch_avgs <- tidy_catch |> 
  bind_rows(dbl_effort_2019) |> 
  group_by(year, survey_date) |> 
  summarize(so_total = mean(so_total, na.rm = TRUE)) |> 
  ungroup() |> 
  group_by(year) |> 
  mutate(percent = cumsum(so_total / sum(so_total) * 100),
         non_cumul_prop = so_total / sum(so_total)) |> 
  ungroup() |> 
...
# Let's start by defining some initial estimates
Asym <- max(df$percent)   # the maximum observed percent catch
xmid <- median(df$yday)   # the median day of the year
scal <- 1                 # this is a rough guess, you may need to adjust

# Now we'll fit the logistic growth curve
fit <- nls(percent ~ Asym / (1 + exp((xmid - yday)/scal)), 
           data = df, 
...
Asym <- max(current$percent)   # the maximum observed percent catch
xmid <- median(current$yday)   # the median day of the year
scal <- 1                 # this is a rough guess, you may need to adjust

# Now we'll fit the logistic growth curve
fit2 <- nls(percent ~ Asym / (1 + exp((xmid - yday)/scal)), 
           data = current, 
           start = list(Asym=Asym, xmid=xmid, scal=scal))
...
# Generate a sequence of days to represent the fitted curve
newdata <- data.frame(yday = seq(min(df$yday), max(df$yday), length.out = 100))

# Predict the percent catch for these new data using the fitted model
newdata$predicted_percent <- predict(fit, newdata = newdata)
newdata$predicted_current <- predict(fit2, newdata = newdata)
...
ggplot(data = newdata) +
  #geom_point(data = df, aes(x = yday, y=percent), colour = "red") +
  #geom_point(data = current, aes(x = yday, y=percent), colour = "blue") +
  geom_line(aes(yday, predicted, linetype = period), size = 1.) +
  labs(x = 'Date', y = 'Cumulative Catch %') +
  scale_colour_discrete(name  = "",
                            breaks=c("predicted_current", "predicted_percent"),
                            labels=c("2023", "2015-2022")) +
...
species_prop_total$species <- species_prop_total$species |> 
  fct_relevel("so_total", "pi_total", "cu_total", "co_total", "he_total", "ck_total") |> 
  fct_recode("Sockeye" = "so_total", "Pink" = "pi_total", "Chum" = "cu_total", "Coho" = "co_total",
             "Herring" = "he_total", "Chinook" = "ck_total")
...
species_prop_total_graph <-  
  ggplot(data = species_prop_total, aes(x = year, y = proportion, fill = species)) +
      ...
      xlab("Year") +
      ylab("Proportion") +
      scale_fill_manual(values = hakai_palette("hakai_salmon", n = 6)) +
      labs(fill = "Species") +

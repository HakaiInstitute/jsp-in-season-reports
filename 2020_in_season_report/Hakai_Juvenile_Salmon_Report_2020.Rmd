...
```{r sealice, echo = FALSE, warning=FALSE, message=FALSE, error=FALSE}

# sealice_field_GH <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/data/sealice_field.csv") %>% 
#   mutate(lice_count = rowSums(.[2:14], na.rm = TRUE),
#          lice_presence = ifelse(lice_count > 0, "P", "A")
#   ) %>% 
#   select(ufn, lice_presence)
# 
# fish_lice_GH <- left_join(sealice_field_GH,
#                           select(ss_fish_GH,
#                                  ufn, sampling_week, survey_date, region, species),
#                           by = "ufn"
# ) %>% 
#   mutate(year = "2015-2019")
...
fish_lice_2020 <- ss_fish_2020 %>% 
  ...
  select(ufn, lice_presence, sampling_week, survey_date, region, species) %>% 
  group_by(sampling_week, species, lice_presence) %>% 
  summarize(count=n()) %>% 
  mutate(prop = count/sum(count))

# fish_lice_field <- bind_rows(fish_lice_2020, fish_lice_GH) %>% 
#   filter(species %in% c("SO", "PI", "CU"), region == "DI",
#          sampling_week %in% c("May 19", "May 26", "June 2", "June 9", "June 16", "June 23", "June 30")) %>% 
#   group_by(sampling_week, species, year, lice_presence) %>% 
#   summarize(count=n()) %>% 
#   mutate(prop = count/sum(count),
#          sampling_week = ordered(sampling_week, levels = c("May 19", "May 26", "June 2", "June 9", "June 16", "June 23", "June 30")))

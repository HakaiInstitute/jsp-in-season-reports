...
# Remove intermediate cols
  select(survey_date, seine_id, ufn, species,lep_cope, lam, laf, lgf, cal_cope, cam, caf, cgf, inferred_leps,
         inferred_cal) |> 
...
# Assign species
         louse_species = if_else(louse_species %in% c("cal_cope", "inferred_cal", "cam", "caf", "cgf"), "Caligus clemensi", "Lepeophtheirus salmonis")) |> 
...
#TODO: Figure out why 2020 and 2021 isn't appearing
...
lab_mots_ts <- read_csv("https://raw.githubusercontent.com/HakaiInstitute/jsp-data/master/supplemental_materials/report_data/sealice_time_series.csv") |> 
  ...
  select(year, survey_date, species, motile_lice = n_lice)
...
sealice_df <- bind_rows(sealice_lab_fs, current_lice, sealice_field) |>
  filter(!species == "CO") |> 
  drop_na(count) |>
  mutate(year = year(survey_date)) |> 
  ungroup() |> 
  ...
  left_join(hosts_observed, by = c('year', 'species')) 
